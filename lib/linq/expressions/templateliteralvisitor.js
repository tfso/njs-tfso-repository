    ...
    evaluate(expression, it = null) {
        ...
        switch (expression.type) {
            case expression_1.ExpressionType.TemplateLiteral:
                let elements = expression.elements.map(el => this.evaluate(el, it));
                if (it) {
                    if (elements.every(el => el.type == expression_1.ExpressionType.Literal) == true)
                        return new literalexpression_1.LiteralExpression(elements.reduce((out, el) => out += this._wrapper(el.value), ''));
                }
                return new templateliteralexpression_1.TemplateLiteralExpression(elements);
