...
const methodexpression_1 = require("../expressions/methodexpression");
...
class WhereOperator extends operator_1.Operator {
    ...
    *evaluate(items) {
    ...
    getExpressionGroups() {
        let me = this, it = this._it, visit = function* (expression) {
            let visitGroup = function* (child) {
                switch (child.type == expression_1.ExpressionType.Logical ? child.operator : -1) {
                    case logicalexpression_1.LogicalOperatorType.Or:
                        break;
                    case logicalexpression_1.LogicalOperatorType.And:
                                ...
                                case expression_1.ExpressionType.Method:
                                    if (me.predicateType == 'OData') {
                                        switch (expr.name) {
                                            case 'tolower':
                                            case 'toupper':
                                                return reduceMemberToIdentifier(expr.parameters[0]);
                                            case 'contains': // bool contains(string p0, string p1)
                                            case 'substringof': { // bool substringof(string po, string p1)                                   
                                                let left = reduceMemberToIdentifier(expr.parameters[0]), right = reduceMemberToIdentifier(expr.parameters[1]);
                                                if (right.type == expression_1.ExpressionType.Literal) {
                                                    return new logicalexpression_1.LogicalExpression(logicalexpression_1.LogicalOperatorType.Equal, left, new literalexpression_1.LiteralExpression(`*${right.value}*`));
                                                }
                                                return expr;
                                            }
            ...
            if (expression instanceof logicalexpression_1.LogicalExpression) {
                ...
                if (expression instanceof methodexpression_1.MethodExpression) {
