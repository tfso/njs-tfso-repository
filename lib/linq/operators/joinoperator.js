    ...
    *evaluate(outer, inner) {
    ...
    evaluateAsync(outer, inner) {
        return __asyncGenerator(this, arguments, function* evaluateAsync_1() {
            ...
            if (this.indexing === true) {
                try {
                    for (var outer_1 = __asyncValues(outer), outer_1_1; outer_1_1 = yield __await(outer_1.next()), !outer_1_1.done;) {
                        let a = yield yield __await(__await(outer_1_1.value));
                        outerAr.push(a);
                        outerKeys.push(this.getOuterKey(a));
                    }
                }
            ...
            try {
                for (var _d = __asyncValues(inner[Symbol.asyncIterator].call(inner, { keyProperty: this.getPropertyName(this.innerProperty), keys: outerKeys })), _e; _e = yield __await(_d.next()), !_e.done;) {
                    let b = yield yield __await(__await(_e.value));
                    let key, values;
                    if ((values = keyvalues.get(key = this.getInnerKey(b))) == null)
                        keyvalues.set(key, values = []);
                    values.push(b);
            ...
            try {
                for (var outer_2 = __asyncValues(outer), outer_2_1; outer_2_1 = yield __await(outer_2.next()), !outer_2_1.done;) {
                    let a = yield yield __await(__await(outer_2_1.value));
                    let values;
                    switch (this.joinType) {
                        case JoinType.Inner:
                            if (values = keyvalues.get(this.getOuterKey(a)))
                                yield yield __await(this.selector(a, new enumerable_1.default(values)));
                            break;
                        case JoinType.Left:
                            if ((values = keyvalues.get(this.getOuterKey(a))) == null)
                                values = [];
                ...
                try {
                    if (outer_2_1 && !outer_2_1.done && (_c = outer_2.return)) yield __await(_c.call(outer_2));
                }
