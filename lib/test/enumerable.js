...
describe("When using Enumerable", () => {
    ...
    describe("with async iterable", () => {
        ...
        it("should be able to iterate", async () => {
            let count = 0;
            try {
                for (var _a = __asyncValues(new enumerable_1.default(list()).where(it => it.id > 0)), _b; _b = await _a.next(), !_b.done;) {
        ...
        it("should be able to handle list of items", async () => {
            let hasItems = false;
            try {
                for (var _a = __asyncValues(new enumerable_1.default(list()).where(it => it.id == 3)), _b; _b = await _a.next(), !_b.done;) {
                    let item = await _b.value;
                    hasItems = true;
        ...
        it("should be able to handle list of promises", async () => {
            let hasItems = false;
            try {
                for (var _a = __asyncValues(new enumerable_1.default(list()).where(it => it.id == 5)), _b; _b = await _a.next(), !_b.done;) {
                    let item = await _b.value;
                    hasItems = true;
