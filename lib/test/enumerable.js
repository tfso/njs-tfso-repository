...
describe("When using Enumerable", () => {
    ...
    describe("with copy", () => {
        it("should be able to copy", async () => {
            let original = new enumerable_1.default(cars).where(it => it.location == 'PORSGRUNN'), copy = original.copy();
            assert.equal(copy.toArray().length, 2);
        });
        it("should be able to copy without affect original", async () => {
            let original = new enumerable_1.default(cars).where(it => it.registrationYear >= 2010), copy = original.copy().where(it => it.location == 'PORSGRUNN');
    ...
    describe("with async iterable", () => {
        ...
        it("should be able to iterate", async () => {
            ...
            let count = 0;
            try {
                for (var _b = __asyncValues(new enumerable_1.default(list()).where(it => it.id > 0)), _c; _c = await _b.next(), !_c.done;) {
                    let item = _c.value;
                    count++;
                }
            }
        ...
        it("should be able to handle list of items", async () => {
            ...
            let hasItems = false;
            try {
                for (var _b = __asyncValues(new enumerable_1.default(list()).where(it => it.id == 3)), _c; _c = await _b.next(), !_c.done;) {
                    let item = _c.value;
                    hasItems = true;
        ...
        it("should be able to handle list of promises", async () => {
            ...
            let hasItems = false;
            try {
                for (var _b = __asyncValues(new enumerable_1.default(list()).where(it => it.id == 5)), _c; _c = await _b.next(), !_c.done;) {
                    let item = _c.value;
                    hasItems = true;
    ...
    it("should skip while year is greater than 2010", () => {
        var ar = new enumerable_1.default(cars).skipWhile(it => it.registrationYear >= 2010).toArray();
    ...
    it("should order by a string", () => {
        var ar = new enumerable_1.default(cars).orderBy('location').toArray();
        assert.deepEqual(ar.map(item => item.location), ["BREVIK", "BREVIK", "HEISTAD", "LANGESUND", "LARVIK", "PORSGRUNN", "PORSGRUNN", "SKIEN"]);
    });
    it("should be able to get first element after order by a string", () => {
        var el = new enumerable_1.default(cars).orderBy('location').first();
        assert.equal(el.id, 5);
    });
    it("should slice a portion", () => {
        var ar = new enumerable_1.default(cars).slice(3, 6).toArray();
        assert.ok(ar.length == 3);
        assert.ok(ar[0].id == 4);
    });
    it("shouldn't slice a portion when a token is used", () => {
        var ar = new enumerable_1.default(cars).slice('test').toArray();
    ...
    it("should be able to get first Operator by class and remove it for manual operator handling", () => {
        ...
        skip = query.operations.first(enumerable_1.OperatorType.Skip);
