    ...
    describe("for iterable", () => {
        ...
        describe("Skip", () => {
            ...
            it("it should be able to iterate when it is removed", async () => {
                ...
                skip.remove();
                try {
                    for (var _a = __asyncValues(skip.evaluate(cars)), _b; _b = await _a.next(), !_b.done;) {
    ...
    describe("for asyncIterable", () => {
        ...
        describe("Skip", () => {
            it("it should be able to iterate", async () => {
                let result = [];
                try {
                    for (var _a = __asyncValues(new skipoperator_1.SkipOperator(5).evaluateAsync(cars())), _b; _b = await _a.next(), !_b.done;) {
                        let item = await _b.value;
                        result.push(item);
                    }
                }
            ...
            it("it should be able to iterate when it is removed", async () => {
                ...
                skip.remove();
                try {
                    for (var _a = __asyncValues(skip.evaluateAsync(cars())), _b; _b = await _a.next(), !_b.done;) {
                        let item = await _b.value;
                        result.push(item);
                    }
                }
