...
describe("When using Repository", () => {
    ...
    it("should work with random query using input param", () => __awaiter(this, void 0, void 0, function* () {
        let repo = new CarRepository(), cars = yield new enumerable_1.default(repo).where((it, id) => it.id == id, 6).toArrayAsync();
        assert.ok(repo instanceof baserepository_1.default);
        assert.equal(cars.length, 1);
        assert.equal(cars[0].id, 6);
    }));
