...
describe("When using repository to read all", () => {
    ...
    it("should handle odata using exact query", () => {
        var list = repository.exposeFilters(new enumerable_1.default().where("tolower(location) eq 'porsgrunn'"));
        assert.equal(list.length, 1);
    ...
    it("should handle odata using wildcard (starts) query", () => {
        var list = repository.exposeFilters(new enumerable_1.default().where("startswith(tolower(location), 'porsgr')"));
        assert.equal(list.length, 1);
        assert.equal(list[0].property, 'location');
        assert.equal(list[0].operator, '==');
    ...
    it("should handle odata using wildcard (ends) query", () => {
        var list = repository.exposeFilters(new enumerable_1.default().where("endswith(tolower(location), 'grunn')"));
        assert.equal(list.length, 1);
        assert.equal(list[0].property, 'location');
        assert.equal(list[0].operator, '==');
    ...
    it("should handle odata using wildcard (starts/ends) query", () => {
        var list = repository.exposeFilters(new enumerable_1.default().where("contains(tolower(location), 'orsgru')"));
        assert.equal(list.length, 1);
        assert.equal(list[0].property, 'location');
        assert.equal(list[0].operator, '==');
    ...
    it("should handle odata using wildcard with other queries", () => {
        var list = repository.exposeFilters(new enumerable_1.default().where("reqistrationYear eq 2017 and contains(tolower(location), 'ors')"));
