...
describe("When using TemplateLiteral for ExpressionVisitor", () => {
    ...
    it("should handle different escaping of placeholders etc", () => {
        assert.equal(template.evaluate(template.visitLambda(() => `My answer is \${1} for now`), {}).value, 'My answer is ${1} for now');
        assert.equal(template.evaluate(template.visitLambda(() => `My answer is $\{2} for now`), {}).value, 'My answer is ${2} for now');
        assert.equal(template.evaluate(template.visitLambda(() => `My answer is \$\{3} for now`), {}).value, 'My answer is ${3} for now');
        assert.equal(template.evaluate(template.visitLambda(() => `My value is $4`), {}).value, 'My value is $4');
        assert.equal(template.evaluate(template.visitLambda(() => `My value is \$5`), {}).value, 'My value is \\$5');
    });
});
